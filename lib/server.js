...
var determineLeave = function() {
    var clients = [];
    for (var key in app._clients) {
        var clientsOfKey = app._clients[key];
        for (var id in clientsOfKey) {
            clients.push({
                key: key,
                id: id,
                client: clientsOfKey[id]
            });
        }
    }
...
var requestOnline = function (clients, idx) {
    ...
    var data = clients[idx];
    if (data.client.socket) {
        beatChecker(data, function (suc) {
            if (!suc) {
                try {
                    data.client.socket.close();
                }catch (e) {

                }
                app._removePeer(data.key, data.id);
            }
            requestOnline(clients, idx + 1);
        });
    } else {
        app._removePeer(data.key, data.id);
    }
...
function beatChecker(cdata, cb, retryCount) {

    cdata.client.socket.send(JSON.stringify({type: "BEAT"}));
    var timer = setTimeout(function () {
        cdata.client.socket.removeListener("message", messageHandle);
        timer = null;
        if (retryCount < 3) {
            beatChecker(cdata, cb, retryCount+1)
        }else {
            cb(false);
        }
    }, 5000);
    ...
    cdata.client.socket.on("message", messageHandle);
...
app._configureWS = function(socket, key, id, token) {
  ...
  socket.on('message', function(data) {
    try {
      ...
      if (['LEAVE', 'CANDIDATE', 'OFFER', 'ANSWER', 'EXDATA'].indexOf(message.type) !== -1) {
...
app.getRouter = function () {
  ...
  router.post('/:key/:id/:token/id', function(req, res, next) {
  ...
  router.get('/:key/peers', function(req, res, next) {
    var key = req.params.key;
  ...
  router.post('/:key/:id/:token/offer', jsonParser, handle);

  router.post('/:key/:id/:token/candidate', jsonParser, handle);

  router.post('/:key/:id/:token/answer', jsonParser, handle);

  router.post('/:key/:id/:token/leave', jsonParser, handle);
